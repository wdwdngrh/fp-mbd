...
## 2. Penjelasan Struktur Relasi

* **`pasien` ke `kunjungan` ($1:\infty$):** Seorang pasien bisa datang berkunjung beberapa kali ke klinik untuk berobat.
* **`kunjungan` ke `pemeriksaan` ($1:1$):** Setiap satu kali kunjungan menghasilkan satu catatan pemeriksaan/rekam medis oleh dokter.
* **`pemeriksaan` ke `resep` ($1:1$):** Dari satu pemeriksaan, dokter mengeluarkan satu instruksi resep obat (opsional, namun jika ada bersifat satu-ke-satu).
* **`resep` ke `detail_resep` ke `obat` ($\infty:\infty$ melalui *Junction Table*):** Satu resep bisa terdiri dari banyak obat, dan satu jenis obat bisa diresepkan di banyak resep yang berbeda.
* **`kunjungan` ke `pembayaran` ($1:1$):** Setiap kunjungan akan menghasilkan satu nota pembayaran untuk layanan medis dan obat yang dikonsumsi. Kolom `total_keseluruhan` menggunakan fitur PostgreSQL `GENERATED ALWAYS AS` untuk otomatis menjumlahkan biaya layanan dan obat secara *real-time*.
